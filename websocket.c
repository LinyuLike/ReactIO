

#include <stdio.h>

#include <openssl/sha.h>
#include <openssl/pem.h>
#include <openssl/bio.h>
#include <openssl/evp.h>

#include "reactor.h"


#define GUID  "258EAFA5-E914-47DA-95CA-C5AB0DC85B11"
#define WEBSOCK_KEY_LENGTH	19


struct _nty_ophdr {

	unsigned char opcode:4,
		 rsv3:1,
		 rsv2:1,
		 rsv1:1,
		 fin:1;
	unsigned char payload_length:7,
		mask:1;

} __attribute__ ((packed));

struct _nty_websocket_head_126 {
	unsigned short payload_length;
	char mask_key[4];
	unsigned char data[8];
} __attribute__ ((packed));

struct _nty_websocket_head_127 {

	unsigned long long payload_length;
	char mask_key[4];

	unsigned char data[8];
	
} __attribute__ ((packed));

typedef struct _nty_websocket_head_127 nty_websocket_head_127;
typedef struct _nty_websocket_head_126 nty_websocket_head_126;
typedef struct _nty_ophdr nty_ophdr;



// Base64编码
int base64_encode(char *in_str, int in_len, char *out_str) {    
	BIO *b64, *bio;    
	BUF_MEM *bptr = NULL;    
	size_t size = 0;    

	if (in_str == NULL || out_str == NULL)        
		return -1;    

	b64 = BIO_new(BIO_f_base64());    
	bio = BIO_new(BIO_s_mem());    
	bio = BIO_push(b64, bio);
	
	BIO_write(bio, in_str, in_len);    
	BIO_flush(bio);    

	BIO_get_mem_ptr(bio, &bptr);    
	memcpy(out_str, bptr->data, bptr->length);    
	out_str[bptr->length-1] = '\0';    
	size = bptr->length;    

	BIO_free_all(bio);    
	return size;
}

// 从缓冲区按行读取数据
int readline(char* allbuf,int level,char* linebuf){    
	int len = strlen(allbuf);    

	for (;level < len; ++level)    {        
		if(allbuf[level]=='\r' && allbuf[level+1]=='\n')            
			return level+2;        
		else            
			*(linebuf++) = allbuf[level];    
	}    

	return -1;
}

// 对WebSocket数据进行解码
void demask(unsigned char *data,int len,char *mask){    
	int i;    
	for (i = 0;i < len;i ++)        
		*(data+i) ^= *(mask+(i%4));
}


// 解析收到的WebSocket帧，提取掩码和数据，并对数据进行解码
unsigned char* decode_packet(unsigned char *stream, char *mask, int __attribute__((unused)) length, int *ret) {
	nty_ophdr *hdr =  (nty_ophdr*)stream;
	unsigned char *data = stream + sizeof(nty_ophdr);
	int size = 0;
	int start = 0;
	//char mask[4] = {0};
	int i = 0;

	if ((hdr->mask & 0x7F) == 126) {

		nty_websocket_head_126 *hdr126 = (nty_websocket_head_126*)data;
		size = hdr126->payload_length;
		
		for (i = 0;i < 4;i ++) {
			mask[i] = hdr126->mask_key[i];
		}
		
		start = 8;
		
	} else if ((hdr->mask & 0x7F) == 127) {

		nty_websocket_head_127 *hdr127 = (nty_websocket_head_127*)data;
		size = hdr127->payload_length;
		
		for (i = 0;i < 4;i ++) {
			mask[i] = hdr127->mask_key[i];
		}
		
		start = 14;

	} else {
		size = hdr->payload_length;

		memcpy(mask, data, 4);
		start = 6;
	}

	*ret = size;
	demask(stream+start, size, mask);

	return stream + start;
}

// 将要发送的数据打包秤WebSocket帧
int encode_packet(char *buffer,char *mask, char *stream, int length) {
	nty_ophdr head = {0};
	head.fin = 1;
	head.opcode = 1;
	int __attribute__((unused)) size = 0;

	if (length < 126) {
		head.payload_length = length;
		memcpy(buffer, &head, sizeof(nty_ophdr));
		size = 2;
	} else if (length < 0xffff) {
		nty_websocket_head_126 hdr = {0};
		hdr.payload_length = length;
		memcpy(hdr.mask_key, mask, 4);

		memcpy(buffer, &head, sizeof(nty_ophdr));
		memcpy(buffer+sizeof(nty_ophdr), &hdr, sizeof(nty_websocket_head_126));
		size = sizeof(nty_websocket_head_126);
		
	} else {
		
		nty_websocket_head_127 hdr = {0};
		hdr.payload_length = length;
		memcpy(hdr.mask_key, mask, 4);
		
		memcpy(buffer, &head, sizeof(nty_ophdr));
		memcpy(buffer+sizeof(nty_ophdr), &hdr, sizeof(nty_websocket_head_127));

		size = sizeof(nty_websocket_head_127);
		
	}

	memcpy(buffer+2, stream, length);

	return length + 2;
}

// 握手
int handshake(struct conn *c) {
	char linebuf[1024] = {0};
	int idx = 0;
	char sec_data[128] = {0};
	char sec_accept[32] = {0};

	do {
		memset(linebuf, 0, 1024);
		idx = readline(c->rbuffer, idx, linebuf);

        // 寻找Sec-WebSocket-Key
		if (strstr(linebuf, "Sec-WebSocket-Key")) {

            // 将Key与GUID拼接
			strcat(linebuf, GUID);
			
            // 计算SHA-1哈希值
			SHA1((const unsigned char*)(linebuf + WEBSOCK_KEY_LENGTH), 
                strlen(linebuf + WEBSOCK_KEY_LENGTH), 
                (unsigned char*)sec_data);

            // 进行base64编码
			base64_encode(sec_data, strlen(sec_data), sec_accept);

            // 生成握手响应
			memset(c->wbuffer, 0, BUFFER_LENGTH); 

			c->wlength = sprintf(c->wbuffer, "HTTP/1.1 101 Switching Protocols\r\n"
					"Upgrade: websocket\r\n"
					"Connection: Upgrade\r\n"
					"Sec-WebSocket-Accept: %s\r\n\r\n", sec_accept);

			printf("ws response : %s\n", c->wbuffer);

			break;
		}
	} while((c->rbuffer[idx] != '\r' || c->rbuffer[idx+1] != '\n') && idx != -1 );

	return 0;
}

// 请求处理
int ws_request(struct conn *c) {
    printf("ws_request called, status: %d\n", c->status);
	printf("request: %s\n", c->rbuffer);

    // 根据连接状态处理请求
	if (c->status == 0) {
		// 状态0，执行握手
		handshake(c);
		c->status = 1;
		
	} else if (c->status == 1) {
        // 状态1：解码数据帧，也就是读取请求
		char __attribute__((unused)) mask[4] = {0};
		int ret = 0;

		c->payload = (char*)decode_packet((unsigned char*)c->rbuffer, c->mask, c->rlength, &ret);

		printf("data : %s , length : %d\n", c->payload, ret);

		c->wlength = ret;

		c->status = 2;
	}

	return 0;
}

// 生成响应
int ws_response(struct conn *c) {
    printf("ws_response called, status: %d\n", c->status);
    // 根据连接状态处理响应，也就是生成响应
	if (c->status == 2) {
		// 状态2：编码数据帧
		c->wlength = encode_packet(c->wbuffer, c->mask, c->payload, c->wlength);
		c->status = 1;
	}

	return 0;
}
